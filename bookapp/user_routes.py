...
from bookapp.models import db, Book, Category, User,Reviews,Donation
...
def profile():
    ...
    if request.method =='GET':
        ...
        if pform.validate_on_submit():
            ...
            picture = request.files.get('pix') #or pform.pix.data.filename
...
def paystack():
    if session.get('ref') !=None:
        ...
        headers = {"Content-Type": "application/json","Authorization":"Bearer sk_test_345ef7cab36c13cfa6da9a3f5a570608a6e132f1"}
...
def home():
    ...
    useronline = session.get('userid')
    userdeets = db.session.query(User).get(useronline)    #fetch all the books
...
def register():
    ...
    if request.method =='GET':
        ...
        if signupform.validate_on_submit():
            ...
            session['user_loggedin'] = True
            return redirect("/dashboard")
            #flash("An account has been created for you")# you do this if you want te person to login after signing up
            #return redirect("/login")
        else:
            return render_template("user/signup.html", signupform=signupform)
